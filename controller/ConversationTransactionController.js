const conversationTransactionModel = require('../model/ConversationTransaction');
  ...
  if (!req.file) {
    ...
    try {
      ...
      const promises = data.map((row) =>
        conversationTransactionModel.uploadFileContents(row)
      );
      ...
      if (!uploadContents.every(Boolean)) {
        res
          .status(401)
          .json({ error: 'There was some issue with file upload' });
      } else {
...
exports.getConversationTransactionData = async (req, res) => {
  try {
    const conversationTransactionData =
      await conversationTransactionModel.getConversationTransactionData();
    console.log('Inside Customer Data');
    res.status(200).json(conversationTransactionData);
  } catch (error) {
